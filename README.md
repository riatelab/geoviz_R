...
`d3carto` is an R package for thematic mapping. As its name suggests, it's an htmlwidget based on the [d3.js](https://d3js.org/) ecosystem ported by Mike Bostock. More precisely, it is an implementation of the [geoviz](https://riatelab.github.io/geoviz/) JavaScript library developed by the [RiateLab](https://riate.cnrs.fr/riatelab.html). The package can be used to create a wide range of interactive, zoomable vector maps, taking advantage of d3's many features: proprtional symbols, pictograms, typologies, choropleth maps, Spikes, tiles, Droing cartograms, etc.
