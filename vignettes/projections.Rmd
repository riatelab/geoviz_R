...
The projections in `geovizr` are based on the ecosystem of `D3.js`. More specifically, they rely on the geographic projection functions implemented in `D3`, which are originally designed to be used in JavaScript.
...
In `geovizr`, the projection is defined in the `viz_create()` function using the `projection` parameter. You simply need to specify a string corresponding to the desired projection.
