# geoviz ![](reference/figures/rgeoviz.svg)
...
geoviz is an R package for thematic mapping. It’s an R wrapper around
...
You can install the released version of `geoviz` from CRAN with:

``` r
install.packages("geoviz")
```

Alternatively, you can install the development version of `geoviz` from
...
Creating a map with `geoviz` requires chaining several functions. The
`create()` function initializes a map with general parameters. Then,
functions such as `outline()`, `graticule()`, `path()`, and many others
...
such as [mapsf](https://CRAN.R-project.org/package=mapsf) or
[tmap](https://CRAN.R-project.org/package=tmap). It really focuses on
the html / interactive geovisualisation.
