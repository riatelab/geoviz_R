...
SVG and the [D3.js](https://d3js.org/) library. As a result, some
argument names may be slightly confusing for R users. In addition, the
package is designed for relatively lightweight basemaps and is not
intended for handling large datasets.
...
functions is available for this purpose. For instance, `viz_path` adds a
spatial dataframe, `viz_graticule` draws latitude and longitude lines,
`viz_header` inserts a title, and `viz_footer` adds a source note. All
available functions are documented in the Reference section.
...
viz_path(data = cities, pointRadius = 2, 
         stroke = "none", fill = "#38896F") |>
viz_header(text = "Hello World", fill = "white", 
